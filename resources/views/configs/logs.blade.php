    ...
    <div class="space-y-6">
        ...
        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4">
            ...
            @if ($config->total_processed > 0)
                <div class="mt-3">
                    <div class="text-sm text-blue-700 mb-1">نرخ موفقیت کلی:
                        {{ round(($config->total_success / $config->total_processed) * 100, 1) }}%</div>
                    <div class="w-full bg-blue-200 rounded-full h-2">
                        <div class="bg-blue-600 h-2 rounded-full"
                            style="width: {{ round(($config->total_success / $config->total_processed) * 100, 1) }}%">
                        </div>
                    </div>
                </div>
            @endif
                        ...
                        @foreach ($logs as $log)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    @if ($log->finished_at)
                                        <div class="text-xs text-gray-400">تا {{ $log->finished_at->format('H:i:s') }}
                                        </div>
                                    @endif
                                </td>
                                <td class="px-4 py-3">
                                    @if ($log->execution_time)
                                        ...
                                        @if ($log->execution_time > 60)
                                            <div class="text-xs text-gray-500">≈{{ round($log->execution_time / 60, 1) }}
                                                دقیقه</div>
                                        @endif
                                    ...
                                    <div class="text-xs space-y-1">
                                        @if ($log->status === 'completed' || $log->status === 'stopped')
                                            @if ($log->total_processed > 0)
                                                <div class="grid grid-cols-2 gap-2">
                                                    <div>کل: <span
                                                            class="font-medium">{{ number_format($log->total_processed) }}</span>
                                                    </div>
                                                    <div>✅ موفق: <span
                                                            class="font-medium text-green-600">{{ number_format($log->total_success) }}</span>
                                                    </div>
                                                    @if ($log->total_duplicate > 0)
                                                        <div>🔄 تکراری: <span
                                                                class="font-medium text-yellow-600">{{ number_format($log->total_duplicate) }}</span>
                                                        </div>
                                                    @endif
                                                    @if ($log->total_failed > 0)
                                                        <div>❌ خطا: <span
                                                                class="font-medium text-red-600">{{ number_format($log->total_failed) }}</span>
                                                        </div>
                                                    @endif
                                                </div>
                                                <div class="pt-1 border-t border-gray-200">
                                                    <div>نرخ موفقیت: <span
                                                            class="font-medium">{{ $log->success_rate }}%</span></div>
                                        ...
                                        @elseif($log->status === 'failed')
                                            ...
                                            @if ($log->error_message)
                                                <div class="text-xs text-red-500">{{ Str::limit($log->error_message, 60) }}
                                                </div>
                                            @endif
                                        @else
                                            <div class="text-yellow-600">در حال پردازش...</div>
                                            @if ($log->total_processed > 0)
                                                <div class="text-xs">تاکنون: {{ $log->total_success }} از
                                                    {{ $log->total_processed }}</div>
                                            @endif
                                        @endif
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                    @if ($log->status === 'completed')
                                        <span class="px-2 py-1 text-xs bg-green-100 text-green-800 rounded">✅ تمام
                                            شده</span>
                                    @elseif($log->status === 'failed')
                                        <span class="px-2 py-1 text-xs bg-red-100 text-red-800 rounded">❌ ناموفق</span>
                                    @elseif($log->status === 'stopped')
                                        <span class="px-2 py-1 text-xs bg-orange-100 text-orange-800 rounded">⏹️ متوقف
                                            شده</span>
                                    @elseif($log->status === 'running')
                                        <span
        ...
        @if ($totalLogs > 0)
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                ...
                <div class="text-xs text-gray-700 space-y-1">
                    <div>• <strong>کل اجراها:</strong> {{ $totalLogs }} بار ({{ $completedLogs }} موفق،
                        {{ $stoppedLogs }} متوقف، {{ $failedLogs }} ناموفق)</div>
                    ...
                    @if ($totalProcessedBooks > 0)
                        <div>• <strong>نرخ موفقیت کلی:</strong>
                            {{ round(($totalSuccessfulBooks / $totalProcessedBooks) * 100, 1) }}%</div>
                    @endif
                </div>
            </div>
        @endif
